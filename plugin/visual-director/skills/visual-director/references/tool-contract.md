...
`visual.configure_project` validates and binds a known project to a local repository directory for the current MCP server process. HTTP MCP sessions within that process share the binding. It is runtime-only: it does not write the repository or persist across restart or across another worker/process. If routing reaches another process, use startup configuration or an explicitly supplied `scene_context.repository_path`; do not infer a path or configure a project that the user did not identify.
