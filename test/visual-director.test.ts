...
import { approvedAnchorPaths } from '../src/projects/canon/adapter.js';
...
describe('Project registry runtime configuration', () => {
  it('binds a repository-native project for subsequent preparation', async () => {
  ...
  it('rejects an unreadable repository path before changing the runtime binding', async () => {
    ...
    await expect(registry.configureProject('bottom-of-thirst', path.join(fixtureRoot, 'missing-repository')))
      .rejects.toMatchObject({ code: 'PROJECT_REPOSITORY_INVALID' });
...
describe('MCP tool', () => {
  it('exposes visual.prepare_generation through the MCP protocol', async () => {
    ...
    try {
      ...
      expect(result.structuredContent).toMatchObject({ project_id: 'bottom-of-thirst', asset_type: 'event_cg' });
      const content = result.content as Array<{ type: string; text?: string }>;
      expect(JSON.parse(content[0]?.text ?? '{}')).toMatchObject({ project_id: 'bottom-of-thirst', asset_type: 'event_cg' });
    } finally {
      await client.close();
      await server.close();
    }
...
describe('multi-project registry', () => {
  it('loads an additional game from a legacy project config without changing the MCP contract', async () => {
