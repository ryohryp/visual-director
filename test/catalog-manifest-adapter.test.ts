...
function catalogEntry(projectId = 'game-a') {
  return parseProjectCatalog({
    projects: [{ project_id: projectId, display_name: projectId === 'bottom-of-thirst' ? 'The Bottom of Thirst' : 'Game A', repository: projectId === 'bottom-of-thirst' ? 'ryohryp/---The-Bottom-of-Thirst' : 'owner/game-a', ref: 'main', adapter_type: 'generic' }],
  }).resolve(projectId);
}
...
describe('catalog repository manifest adapter', () => {
  it('creates a generic adapter from the repository-owned manifest', async () => {
    ...
    writeFileSync(path.join(root, '.visual-director', 'manifest.json'), JSON.stringify({ version: 1, project_id: 'game-a', subjects: {} }), 'utf8');
  ...
  it('rejects a repository manifest for a different project', async () => {
    ...
    mkdirSync(path.join(root, '.visual-director'));
    writeFileSync(path.join(root, '.visual-director', 'manifest.json'), JSON.stringify({ version: 1, project_id: 'other-game' }), 'utf8');
    await expect(createCatalogProjectAdapter(catalogEntry(), new LocalRepositorySource(root))).rejects.toThrow('Project manifest project_id does not match catalog project_id.');
  });

  it('prepares asset-aware Grand Design contracts while preserving new-anchor compatibility', async () => {
    ...
    const files: Record<string, string | Buffer> = {
      '.visual-director/manifest.json': JSON.stringify({
        ...
        documents: { grandDesign: '.visual-director/grand-design.json', globalStyle: 'docs/visual/GLOBAL_VISUAL_STYLE.md', characterCanon: 'docs/visual/CHARACTER_VISUAL_CANON.md', worldDirection: 'docs/WORLD_DIRECTION.md', assetManifest: 'docs/visual/assets/README.md', globalReference: 'docs/visual/assets/global_visual_style_reference.webp' },
        labels: { avoidBlockHeading: 'Fixed Avoid Block', allowedChangesHeading: '変更してよいもの', forbiddenChangesHeading: '変更してはいけないもの', commonRulesHeading: '共通ルール', acceptedConditionsHeading: '採用する視覚条件' },
        subjects: {
          kamino_kyosuke: { display_name: '神野 恭介', character_file: 'docs/characters/kyosuke.md', canon_heading: '神野 恭介', aliases: ['神野', '恭介', 'Kamino Kyosuke'] },
          mikoshiba: { display_name: '御子柴 徹', character_file: 'docs/characters/mikoshiba.md', canon_heading: '御子柴 徹', aliases: ['御子柴'], anchor_requirements_file: 'docs/visual/MIKOSHIBA_VISUAL_ANCHOR_V2_REQUIREMENTS.md', required_new_anchor_terms: ['25歳', '刑事'] },
        },
      }),
      '.visual-director/grand-design.json': JSON.stringify({ schema_version: 1, project_id: 'bottom-of-thirst', visual_dna: 'ordinary first', shared_rules: { practical_light: true }, fixed_avoid: ['generic horror'], asset_types: { event_cg: { purpose: 'evidence', forbidden: ['invent facts'] }, background: { purpose: 'observation base' } } }),
    ...
    expect(prepared.prompt_package.grand_design_contract).toEqual(expect.objectContaining({ asset_type: 'event_cg', asset_contract: { purpose: 'evidence', forbidden: ['invent facts'] } }));
    expect(JSON.stringify(prepared.prompt_package.grand_design_contract)).not.toContain('observation base');
    expect(prepared.prompt_package.grand_design_lock).toBeUndefined();

    const newAnchor = await adapter.prepare({ project_id: 'bottom-of-thirst', asset_type: 'visual_anchor', subject_ids: ['御子柴'], request_text: '御子柴徹の初期Approved Visual Anchor候補を作る。' });
    expect(newAnchor.asset_type).toBe('character_visual_anchor');
    expect(newAnchor.policy.must_use_approved_anchor).toBe(false);
    expect(newAnchor.prompt_package.grand_design_contract).toBeUndefined();
    expect(newAnchor.prompt_package.grand_design_lock).toBeUndefined();
    ...
    expect(overview.approved_anchors).toContainEqual({ subject_id: 'kamino_kyosuke', display_name: '神野 恭介', asset_type: 'character_visual_anchor', path: 'public/images/characters/kamino_kyosuke/v2/default.avif', status: 'approved' });
