...
describe('compiled Canon', () => {
  it('writes deterministic repository-native Crownless Canon including manifest grand design', async () => {
    ...
    expect(first.grand_design).toContain('Crownless Grand Design');
    expect(first.grand_design).toContain('silhouette-first readability');
  ...
  it('fails --check when Canon-derived output is stale', async () => {
    ...
    await writeFile(path.join(repositoryPath, 'docs/visual/GRAND_DESIGN.md'), '# Changed Grand Design\n', 'utf8');
...
async function crownlessRepository(): Promise<string> {
  ...
  await writeFile(path.join(repositoryPath, '.visual-director/manifest.json'), JSON.stringify({
    ...
    documents: {
      grandDesign: 'docs/visual/GRAND_DESIGN.md',
  ...
  await writeFile(path.join(repositoryPath, 'docs/visual/GRAND_DESIGN.md'), '# Crownless Grand Design\n\n- silhouette-first readability\n- restrained medieval manuscript composition\n', 'utf8');
