...
interface LocalCompileRuntime { overview: ProjectVisualOverview; prepare(subjectId: string): Promise<GenerationPackage>; }
...
export async function compileRepositoryCanon(input: CompileRepositoryCanonInput): Promise<CompiledCanon> {
  ...
  const semanticPayload = {
    ...
    ...(baseline.prompt_package.grand_design_lock ? { grand_design: baseline.prompt_package.grand_design_lock } : {}),
  ...
  if (input.check) {
    ...
    try { current = await readFile(outputPath, 'utf8'); } catch (error) {
      throw new VisualDirectorError('COMPILED_CANON_STALE', 'Compiled Canon is missing or unreadable.', { path: path.relative(repositoryPath, outputPath).replace(/\\/g, '/'), reason: error instanceof Error ? error.message : String(error) });
    }
...
async function runtimeFromAdapter(adapter: ProjectAdapter, projectId: string): Promise<LocalCompileRuntime> {
  return { overview: await adapter.getVisualOverview(), prepare: (subjectId) => adapter.prepare({ project_id: projectId, asset_type: 'character_visual_anchor', subject_ids: [subjectId], request_text: 'Compile repository Canon.' }) };
}
...
function resolveCompiledCanonOutputPath(repositoryPath: string, outputPath?: string): string {
  ...
  if (!relativePath || path.isAbsolute(relativePath) || /^[a-zA-Z]:/.test(relativePath) || relativePath.split('/').includes('..')) throw new VisualDirectorError('COMPILED_CANON_OUTPUT_INVALID', 'Compiled Canon output path must be repository-relative and stay inside the repository.', { output_path: outputPath ?? COMPILED_CANON_RELATIVE_PATH });
...
function sortJson(value: unknown): unknown {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).sort(([left], [right]) => left.localeCompare(right)).map(([key, nested]) => [key, sortJson(nested)]));
}
