...
export async function compileCanonFromSource(projectId: string, source: RepositorySource): Promise<CompiledCanon> {
  ...
  const grandDesign = definition.documents.grandDesign
    ...
    : undefined;
  return compileCanonFromRuntime(projectId, {
    ...(await runtimeFromAdapter(adapter, projectId)),
    ...(grandDesign ? { grandDesign } : {}),
  });
}

export function serializeCompiledCanon(compiled: CompiledCanon): string {
  return `${stableJson(compiled)}\n`;
}

async function compileCanonFromRuntime(projectId: string, runtime: LocalCompileRuntime): Promise<CompiledCanon> {
  if (runtime.overview.approved_anchors.length === 0) throw new VisualDirectorError('COMPILED_CANON_NO_APPROVED_ANCHORS', 'Compiled Canon requires at least one Approved Visual Anchor so the repository-native flow can remain fail-closed.', { project_id: projectId });
  ...
  return { schema_version: 1, source_revision: createHash('sha256').update(stableJson(semanticPayload)).digest('hex'), ...semanticPayload };
}

async function createLegacyLocalCompileRuntime(projectId: string, repositoryPath: string): Promise<LocalCompileRuntime> {
  ...
  return {
    ...
    prepare: (subjectId) => core.prepareGeneration({ project_id: projectId, asset_type: 'character_visual_anchor', subject_ids: [subjectId], request_text: 'Compile repository Canon.', scene_context: { repository_path: repositoryPath } }),
  };
}
