...
import type { AdoptAnchorInput, AdoptAnchorResult, OpenAIFileReference, ProjectAdapter } from '../domain/types.js';
import { CanonProjectAdapter, DEFAULT_PROJECT_DOCUMENTS, DEFAULT_PROJECT_LABELS } from './canon/adapter.js';
import { DEFAULT_CANON_MANIFEST_PATH, parseRepositoryCanonManifest } from './canon/repository-manifest.js';
import type { CanonProjectDefinition, ProjectDocuments, ProjectLabels, ProjectSubjectDefinition } from './canon/types.js';
...
export interface ProjectRegistryOptions {
  /** @deprecated Use Project Registry or configureProject. Kept for local compatibility only. */
  repoPath?: string;
...
interface ProjectConfigFile { projects: Record<string, ProjectConfig>; }
...
export function createProjectRegistry(options: ProjectRegistryOptions = {}): ProjectRegistry {
  const legacyDefinitions = new Map<string, CanonProjectDefinition>([
  ...
  // Compatibility only: the old single-repository option never carries Canon data.
  if (options.repoPath) repoPaths.set('bottom-of-thirst', path.resolve(options.repoPath));
  ...
  if (options.projectsConfigPath) {
    ...
    for (const [projectId, rawProjectConfig] of Object.entries(config.projects) as Array<[string, unknown]>) {
      ...
      legacyDefinitions.set(projectId, normalizeDefinition(projectId, projectConfig));
      repoPaths.set(projectId, resolveConfigPath(options.projectsConfigPath, projectConfig.repo_path));
    }
  }

  const definitionFor = (projectId: string): CanonProjectDefinition => {
    const repoPath = repoPaths.get(projectId);
    if (repoPath) {
      const manifest = tryReadRepositoryManifest(repoPath);
      if (manifest) {
        if (manifest.projectId !== projectId) {
          throw new VisualDirectorError('PROJECT_MANIFEST_INVALID', 'Project manifest project_id does not match configured project_id.', {
            project_id: projectId,
            manifest_project_id: manifest.projectId,
          });
        }
        return manifest;
      }
    }
    const legacy = legacyDefinitions.get(projectId);
    if (legacy) return legacy;
    throw new VisualDirectorError('PROJECT_NOT_FOUND', `Unsupported project_id: ${projectId}.`, {
      known_project_ids: [...new Set([...legacyDefinitions.keys(), ...repoPaths.keys()])],
    });
  };
  ...
  return {
    async configureProject(projectId: string, repositoryPath: string): Promise<ProjectConfiguration> {
      ...
      if (!trimmedPath) {
        throw new VisualDirectorError('PROJECT_CONFIG_INVALID', 'repository_path must not be empty.', { project_id: normalizedProjectId });
      }
      ...
      const manifest = tryReadRepositoryManifest(resolvedPath);
      if (manifest && manifest.projectId !== normalizedProjectId) {
        throw new VisualDirectorError('PROJECT_MANIFEST_INVALID', 'Project manifest project_id does not match configured project_id.', {
          project_id: normalizedProjectId,
          manifest_project_id: manifest.projectId,
        });
      }
      if (!manifest && !legacyDefinitions.has(normalizedProjectId)) {
        throw new VisualDirectorError('PROJECT_MANIFEST_INVALID', `Repository does not contain ${DEFAULT_CANON_MANIFEST_PATH}.`, {
          project_id: normalizedProjectId,
          repository_path: resolvedPath,
        });
      }

      repoPaths.set(normalizedProjectId, resolvedPath);
      return { project_id: normalizedProjectId, repository_path: resolvedPath, persistence: 'runtime' };
    },
    ...
    async adoptAnchor(input: AdoptAnchorInput): Promise<AdoptAnchorResult> {
      ...
      const repoPath = repoPaths.get(input.project_id);
      if (!repoPath) {
        throw new VisualDirectorError('PROJECT_CONFIG_MISSING', `No repository path is configured for project_id: ${input.project_id}.`);
      }
      const definition = definitionFor(input.project_id);
      const subject = resolveSubject(definition, input.subject_id);
      if (!subject) throw new VisualDirectorError('SUBJECT_NOT_FOUND', `Unknown subject_id: ${input.subject_id}.`);
      ...
      if (hasCandidateFile === hasCandidatePath) {
        throw new VisualDirectorError('AMBIGUOUS_CANDIDATE_INPUT', 'Provide exactly one of candidate_file or candidate_path.');
      }
      ...
      const canonMarkdown = await readFile(canonAbsolutePath, 'utf8');
      ...
      if (hasCandidatePath) {
        ...
        await replaceCanonFile(canonAbsolutePath, registerApprovedAnchor(canonMarkdown, subject.canonHeading, candidatePath), 'CANON_WRITE_FAILED');
      ...
      try {
        ...
        committed = true;
        await replaceCanonFile(canonAbsolutePath, registerApprovedAnchor(canonMarkdown, subject.canonHeading, anchorPath), 'CANON_WRITE_FAILED');
      } catch (error) {
      ...
      return buildAdoptAnchorResult(input.project_id, subject.id, anchorPath, canonPath, true, downloaded.bytes, downloaded.image);
    },
    ...
    resolve(projectId: string): ProjectAdapter {
      ...
      if (projectId === personalOrbitDefinition.projectId && !tryReadRepositoryManifest(repoPath)) {
        return new PersonalOrbitAdapter(personalOrbitDefinition, { repoPath });
      }
      return new CanonProjectAdapter(definitionFor(projectId), { repoPath });
    },
  };
}

function tryReadRepositoryManifest(repoPath: string): CanonProjectDefinition | undefined {
  const manifestPath = path.join(repoPath, DEFAULT_CANON_MANIFEST_PATH);
  let raw: string;
  try {
    raw = readFileSync(manifestPath, 'utf8');
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined;
    throw new VisualDirectorError('PROJECT_MANIFEST_INVALID', 'Visual Director project manifest could not be read.', {
      path: manifestPath,
      reason: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  return parseRepositoryCanonManifest(parsed, DEFAULT_CANON_MANIFEST_PATH);
}

function resolveSubject(definition: CanonProjectDefinition, value: string): ProjectSubjectDefinition | undefined {
  const normalized = normalizeAlias(value);
  return Object.values(definition.subjects).find((subject) =>
    [subject.id, subject.displayName, ...(subject.aliases ?? [])].some((candidate) => normalizeAlias(candidate) === normalized));
}
...
interface RepositoryRoot { absolutePath: string; realPath: string; }
interface DownloadedCandidate { bytes: Buffer; image: ReturnType<typeof inspectImage>; }
...
function isWithinOrEqual(parent: string, candidate: string): boolean { return parent === candidate || isWithin(parent, candidate); }

async function ensureExistingRepositoryFile(repository: RepositoryRoot, absolutePath: string, code: string, relativePath: string): Promise<void> {
  if (!isWithinOrEqual(repository.absolutePath, absolutePath)) {
    throw new VisualDirectorError('UNSAFE_REPOSITORY_PATH', 'A repository path resolves outside the configured repository.', { path: relativePath });
  }
  try {
    const fileStats = await lstat(absolutePath);
    if (fileStats.isSymbolicLink() || !fileStats.isFile()) throw new Error('Path is not a safe regular file.');
    const realFilePath = await realpath(absolutePath);
    if (!isWithin(repository.realPath, realFilePath)) {
      throw new VisualDirectorError('UNSAFE_REPOSITORY_PATH', 'A repository file resolves outside the configured repository.', { path: relativePath });
    }
  } catch (error) {
...
async function ensureSafeDestination(repository: RepositoryRoot, absolutePath: string, relativePath: string): Promise<string[]> {
  if (!isWithin(repository.absolutePath, absolutePath)) {
    throw new VisualDirectorError('UNSAFE_REPOSITORY_PATH', 'The Anchor destination resolves outside the configured repository.', { path: relativePath });
  }
  ...
  while (current !== repository.absolutePath) {
    ...
    if (!isWithinOrEqual(repository.absolutePath, current)) {
      throw new VisualDirectorError('UNSAFE_REPOSITORY_PATH', 'The Anchor destination escapes the configured repository.', { path: relativePath });
    }
  }
...
async function removeEmptyDirectories(directories: string[]): Promise<void> {
  for (const directory of [...directories].reverse()) {
    try { await rm(directory, { recursive: false, force: true }); } catch { /* best effort */ }
  }
}
...
async function replaceFileAtomically(filePath: string, contents: string, errorCode: string): Promise<void> {
  ...
  try {
    ...
    catch (error) { await rename(backupPath, filePath).catch(() => undefined); throw error; }
    await rm(backupPath, { force: true });
  } catch (error) {
    await rm(temporaryPath, { force: true });
    if (backupCreated) {
      try { await lstat(backupPath); await rename(backupPath, filePath).catch(() => undefined); } catch { /* no-op */ }
    }
    if (error instanceof VisualDirectorError) throw error;
...
async function downloadCandidateFile(file: OpenAIFileReference, fetchImpl: typeof fetch): Promise<DownloadedCandidate> {
  ...
  let downloadUrl: URL;
  try { downloadUrl = new URL(file.download_url); }
  catch { throw new VisualDirectorError('FILE_DOWNLOAD_URL_INVALID', 'candidate_file.download_url must be a valid HTTPS URL.'); }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) throw new VisualDirectorError('FILE_DOWNLOAD_FAILED', 'Redirects are not accepted for candidate files.');
    if (!response.ok) throw new VisualDirectorError('FILE_DOWNLOAD_FAILED', 'The candidate file download returned an error.', { status: response.status });
    const contentLength = Number(response.headers.get('content-length'));
    if (Number.isFinite(contentLength) && contentLength > MAX_IMAGE_BYTES) {
      throw new VisualDirectorError('IMAGE_TOO_LARGE', 'The downloaded candidate file exceeds the 25 MiB limit.', { max_bytes: MAX_IMAGE_BYTES });
    }
...
function anchorDestinationPath(subjectId: string, extension: string): string {
  if (!/^[a-zA-Z0-9_-]+$/.test(subjectId)) {
    throw new VisualDirectorError('UNSAFE_REPOSITORY_PATH', 'The subject_id cannot be used to derive an Anchor destination.', { subject_id: subjectId });
  }
...
function findApprovedAnchor(markdown: string, canonHeading: string): string | undefined {
  ...
  const section = markdown.slice(sectionStart, nextHeadingOffset >= 0 ? sectionStart + nextHeadingOffset : markdown.length);
  ...
  return section.slice(afterHeading, nextSubheading >= 0 ? afterHeading + nextSubheading : section.length)
    .match(/^\s*-\s+`([^`]+)`\s*$/m)?.[1];
}

function buildAdoptAnchorResult(projectId: string, subjectId: string, anchorPath: string, canonPath: string, changed: boolean, bytes: Uint8Array, image: ReturnType<typeof inspectImage>): AdoptAnchorResult {
  return {
    project_id: projectId,
...
export function registerApprovedAnchor(markdown: string, canonHeading: string, anchorPath: string): string {
  ...
  if (!headingMatch || headingMatch.index === undefined) {
    throw new VisualDirectorError('CANON_SUBJECT_NOT_FOUND', 'The subject section is missing from the character Visual Canon.', { canon_heading: canonHeading });
  }
  const sectionStart = headingMatch.index + headingMatch[0].length;
  const rest = markdown.slice(sectionStart);
  const nextHeadingOffset = rest.search(/^##\s+/m);
  ...
  if (approvedHeading?.index !== undefined) {
    ...
    return markdown.slice(0, insertionPoint) + `${newline}- \`${anchorPath}\`` + markdown.slice(insertionPoint);
  }
...
function readProjectConfig(configPath: string): ProjectConfigFile {
  ...
  let parsed: unknown;
  try { parsed = JSON.parse(readFileSync(resolvedPath, 'utf8')); }
  catch (error) {
    throw new VisualDirectorError('PROJECT_CONFIG_INVALID', 'Could not read or parse the Visual Director project config.', {
      path: resolvedPath,
      reason: error instanceof Error ? error.message : String(error),
    });
  }
...
function normalizeDefinition(projectId: string, config: ProjectConfig): CanonProjectDefinition {
  ...
  for (const [subjectId, subject] of Object.entries(config.subjects)) {
    ...
    subjects[subjectId] = { id: subjectId, displayName: subject.display_name, characterFile: subject.character_file, canonHeading: subject.canon_heading };
  }
  return { projectId, documents: normalizeDocuments(config.documents), labels: normalizeLabels(config.labels), subjects };
}
...
function resolveConfigPath(configPath: string, configuredPath: string): string { return path.resolve(path.dirname(path.resolve(configPath)), configuredPath); }
