...
export class CanonProjectAdapter extends CanonAdapterBase {
  ...
  protected async loadSubject(subjectId: string, canonMarkdown: string, assetType = ''): Promise<SubjectAnchor> {
    ...
    if (!configured.anchorRequirementsFile) {
      throw new VisualDirectorError('NEW_ANCHOR_REQUIREMENTS_NOT_CONFIGURED', `New Visual Anchor requirements are not configured for ${subjectId}.`, {
        subject_id: subjectId,
      });
    }
  ...
  protected async listApprovedAnchors(canonMarkdown: string): Promise<ApprovedAnchorSummary[]> {
    ...
    for (const configured of Object.values(this.definition.subjects)) {
      ...
      anchors.push({
        ...
        asset_type: NEW_ANCHOR_ASSET_TYPE,
        path: anchorPath,
        status: 'approved',
      });
    }
  ...
  protected forbiddenChanges(canonMarkdown: string, styleMarkdown: string, preparingNewAnchor = false): string[] {
    return [
      ...
      ...(preparingNewAnchor ? [
        ...
        'Do not generate from assistant memory, conversation guesses, or fallback character facts when Canon preparation fails.',
      ] : []),
...
function normalizeAssetType(value: string): string {
  ...
  if (normalized === 'visual_anchor' || normalized === 'character_anchor' || normalized === 'visualanchor') return NEW_ANCHOR_ASSET_TYPE;
  return value.trim();
}
...
function validateRequiredNewAnchorTerms(
  configured: ProjectSubjectDefinition,
  characterMarkdown: string,
  requirementsMarkdown: string,
): void {
  ...
  if (missingTerms.length > 0) {
    throw new VisualDirectorError('NEW_ANCHOR_CANON_INCOMPLETE', `Critical new-anchor Canon facts are missing for ${configured.id}.`, {
      subject_id: configured.id,
