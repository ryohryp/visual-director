...
function validateRequiredNewAnchorTerms(configured: ProjectSubjectDefinition, characterMarkdown: string, requirementsMarkdown: string): void { const requiredTerms = configured.requiredNewAnchorTerms ?? []; const source = `${characterMarkdown}\n${requirementsMarkdown}`; const missingTerms = requiredTerms.filter((term) => !source.includes(term)); if (missingTerms.length > 0) throw new VisualDirectorError('NEW_ANCHOR_CANON_INCOMPLETE', `Critical new-anchor Canon facts are missing for ${configured.id}.`, { subject_id: subject.id, missing_terms: missingTerms }); }
