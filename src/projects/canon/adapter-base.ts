...
import { computeGenerationFingerprint, GENERATION_PACKAGE_SCHEMA_VERSION } from '../../domain/fingerprint.js';
...
export abstract class CanonAdapterBase implements ProjectAdapter {
  ...
  async prepare(input: PrepareGenerationInput): Promise<GenerationPackage> {
    ...
    return {
      ...basePackage,
      fingerprint: computeGenerationFingerprint(basePackage),
    };
  }
  ...
  private sceneRequirements(input: PrepareGenerationInput, worldMarkdown: string): string[] {
    const contextEntries = Object.entries(input.scene_context ?? {}).sort(([a], [b]) => a.localeCompare(b));
