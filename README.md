...
Generation Packageには`schema_version`（初期値は`1`）と`fingerprint`も含まれます。`fingerprint`は`fingerprint`自身を除くPackageの意味内容から算出した、キー順序に依存しないSHA-256のlowercase hex文字列です。同じCanon・request・対象・設定で同じPackageかどうかを判定できます。
